...
fn main() {
    ...
    let matches = app_from_crate!()
                            // Create command:
                            .subcommand(SubCommand::with_name(SUBCOMMAND_SHARE)
...
file.out.s1, file.out.s2, and so on.")
                                ...
                                .arg(Arg::with_name(ARG_NO_CONFIRM_RECON)
                                     .short("n")
                                     .long("no-confirm")
                                     .takes_value(false)
                                     .help("Skip adding a hash to the end of the input to confirm reconstruction")
                                     .required(false)))
    ...
    match args.subcommand() {
        (SUBCOMMAND_SHARE, Some(sub_matches)) => {
            ...
            let out_file_stem = sub_matches.value_of(ARG_OUTPUT_STEM)
                                    .unwrap_or(
                                        Path::new(sub_matches.value_of(ARG_INPUT).unwrap()).file_stem().expect("Invalid input file path.").to_str().unwrap());
            ...
            let shares_needed = sub_matches.value_of(ARG_SHARES_NEEDED).unwrap_or(
                        sub_matches.value_of(ARG_SHARES_TO_CREATE).unwrap()).parse::<u8>().unwrap();
            ...
            let mut dests: Vec<Box<dyn Write>> = (0..(shares_to_create as usize))
                .map(|share_num| {
                    let path = Path::new(out_dir).join(format!("{}_{}.sss", out_file_stem, share_num));
                    Box::new(std::fs::OpenOptions::new()
                        .create(true)
                        .write(true)
                        .open(&path)
                        .expect(&format!("Failed to open file '{}' for writing.", path.as_path().to_str().unwrap()))) as Box<dyn Write>
                })
                .collect();
            share_to_writables(secret, &mut dests, shares_needed, shares_to_create, true).expect("Failed to share secret.");
        ...
        (SUBCOMMAND_RECONSTRUCT, Some(sub_matches)) => {
            ...
            let input_files: Vec<Vec<u8>> = sub_matches.values_of(ARG_INPUTS)
            ...
            secret.reconstruct(input_files, true).expect("Reconstruction failed"); 
